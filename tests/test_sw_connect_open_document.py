...
class _FakeSWWithActiveDoc:
    """SolidWorks mock with an ActiveDoc attribute."""
    ActiveDoc = FakeModel()


class _FakeSWMissingActiveDoc:
    """SolidWorks mock without ActiveDoc (lookup miss)."""
    pass


class _FakeSWActiveDocRaises:
    """SolidWorks mock where accessing ActiveDoc raises a non-AttributeError."""
    @property
    def ActiveDoc(self):
        raise RuntimeError("COM call failed")


def test_read_active_document_returns_doc_when_present(get_com_member):
    """_read_active_document returns the active doc when present."""
    from scripts import sw_connect as sw_connect_module
    sw = _FakeSWWithActiveDoc()
    # Patch get_com_member in the sw_connect module
...
def test_read_active_document_returns_none_on_lookup_miss(get_com_member):
    """_read_active_document returns None when ActiveDoc is missing."""
    from scripts import sw_connect as sw_connect_module
    sw = _FakeSWMissingActiveDoc()
    original_get_com_member = sw_connect_module.get_com_member
    sw_connect_module.get_com_member = get_com_member
    try:
        result = sw_connect_module._read_active_document(sw)
